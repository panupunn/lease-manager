...
def _connect_ws():
    ...
    if "gsheets" not in st.secrets:
        st.error("ไม่พบ [gsheets] ใน Secrets — ไปที่ Manage app → Settings → Secrets แล้ววาง TOML ให้ถูกต้อง")
        st.stop()
...
def filter_by_query(df: pd.DataFrame, q: str) -> pd.DataFrame:
    """
    ค้นหาได้จาก: เลขสัญญา / ชื่อร้าน / ผู้ติดต่อ / เบอร์โทร
    - เบอร์โทร: เทียบเฉพาะ "ตัวเลข" (ตัด 0 นำหน้า/ขีด/เว้นวรรค/วงเล็บออก) เพื่อกันเคสที่ชีตเก็บเป็นตัวเลข
    """
    if not q:
        return df
    ...
    # เบอร์โทร: เทียบตัวเลขล้วน
    q_digits = re.sub(r"\D+", "", q)
    if q_digits:
        phone_norm = df["phone"].astype("string").fillna("").str.replace(r"\D+", "", regex=True)
        mask = mask | phone_norm.str.contains(q_digits, na=False)
    else:
        mask = mask | _contains(df["phone"], q)
...
# -------------------- หน้า: ค้นหา/แจ้งเตือน -----------------
    ...
    with st.expander("ตัวกรองการค้นหา", expanded=True):
        ...
        with c2:
            # ค่าเริ่มต้นเป็น "ทั้งหมด" เพื่อไม่กรองทิ้งโดยไม่ตั้งใจ
            within = st.selectbox("จะแหมดอายุภายใน", ["ทั้งหมด", "15 วัน", "30 วัน", "กำหนดเอง"], index=0)
        with c3:
...
# --------------------- หน้า: ข้อมูลทั้งหมด ------------------
    ...
    if _df.empty:
        ...
        display_cols = [
            "contract_no", "shop_name", "contact_name", "phone",
            "start_date", "months", "end_date", "cancelled", "สถานะ", "id"
        ]
        for c in display_cols:
            if c not in dfv.columns:
                dfv[c] = pd.NA

        st.dataframe(dfv[display_cols], use_container_width=True)
